...
const cleanRules = (rules) => {
  return rules.filter((row) => {
    if (row.version === 'control') {
      ...
      const lastAsNum = makeNumber(row.key.slice(-1));
      if (lastAsNum != lastAsNum) {
...
// to reduct api_key from logs
const reductedAmplitudeBody = {
  api_key: 'reducted',
  events: amplitudeEvents,
};
...
if (loggingEnabled) {
  doLogging('Request', stdLogInfo, {
    ...
    requestBody: reductedAmplitudeBody,
  });
}
